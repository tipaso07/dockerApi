...
router.post('/register', async (req, res) => {
  try {
    ...
    const usuario = new Usuario({ nombre, email, password: hashed, rol: rol || 'Cliente', zona: zona || '' });
    await usuario.save();
    const token = jwt.sign({ id: usuario._id, nombre: usuario.nombre, rol: usuario.rol, zona: usuario.zona }, JWT_SECRET, { expiresIn: '7d' });
